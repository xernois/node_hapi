...
},
{
    ...
    options: {
        ...
        validate: {
            payload: Joi.object({
                titre: Joi.string().min(3).example('titanic').description('Username of the user'),
                description: Joi.string().min(8).example('a boat related movie where there is not enough place on a plank for two.').description('the movie description'),
                date: Joi.string().min(8).example('1989/05/23').description('the movie release date'),
                realisateur: Joi.string().min(8).example('spielberg').description('the movie realisator')
            })
        }
    },
    handler: async (request, h) => {
        ...
        const mails = (await movieService.getUser(request.params.id)).map((user) => user.email);

        if (movie && mails?.length) {
